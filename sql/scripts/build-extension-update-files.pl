...
my $PROJECT = $PROJECT;
my $version_0_1 = qr/(0.1.[\d+])/;
# add minor here

my $version_0 = qr/(0.[\d+].[\d+])/;
# add mayors here
...
my $curr_signature_file_name = "$signature_dir/$PROJECT--$new_minor.sig";
my $old_signature_file_name = "$signature_dir/$PROJECT--$old_minor.sig";
my $curr_sql_file_name = "$output_directory/$PROJECT--$new_version.sql";
my $output_file_name = "$output_directory/$PROJECT--$old_version--$new_version.sql";
...
sub write_script {
    ...
    || die "ERROR: failed to create '$output_directory/$PROJECT-$PROJECT--$old_version--$new_version.sql' : $!\n";
...
-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
";
    my $echo = "\\echo Use \"ALTER extension $PROJECT update to '$new_version'\" to load this file. \\quit";
...
sub get_current_sql {
    ...
    # remove the header
    $contents  =~ s/\\echo Use "CREATE EXTENSION $PROJECT" to load this file. \\quit//;
...
sub drop_special_case_function {
    ...
    push @commands, "ALTER EXTENSION $PROJECT DROP FUNCTION $function;\n";
